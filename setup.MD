...
3. **ファームウェアの更新**
   - すべてのサーボが検出されたことを確認
   - **Upgrade** をクリック
   - **Online Detection** を実行
   - **Upgrade Firmware** を実行
   - ファームウェアバージョンが **3.9** から **3.10** に更新されていることを確認

4. **バージョン確認**
   - 更新後、すべてのサーボのファームウェアバージョンが3.10になっていることを確認
   - これにより以降の動作問題を回避できます

#### モーターID設定（重要）

**ファームウェア更新後、必ずモーターIDを設定してください。**
...
4. **IDの確認**
   - 一度接続を閉じて（Close）、再度Open → Searchを実行
   - 変更したIDでモーターが検出されることを確認
...
キャリブレーションファイル削除後、上記の「5. キャリブレーション」セクションの手順に従ってキャリブレーションを実行します。
...
### 8. テレオペレーション（リーダーアームでフォロワーアームを操作）
